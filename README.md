...
result.variants.sm.raster.blob   // Blob, image/jpeg or image/png  (≤ 640px longest edge)
result.variants.sm.avif.blob     // Blob, image/avif
result.variants.md               // ≤ 1024px
result.variants.lg               // ≤ 1440px
result.variants.org              // ≤ 1920px (largest variant; capped, not full-res)
result.format                    // "jpeg" | "png" — decided once on the source
result.alpha                     // { nonOpaque, total, ratio, isTransparent }
result.source                    // { width, height, mimeType }
```
...
### The fixed variant model

| key   | longest edge        | raster quality        | AVIF quality |
| ----- | ------------------- | --------------------- | ------------ |
| `sm`  | ≤ 640px (default)   | `quality` (default 80) | `max(40, quality−25)` |
| `md`  | ≤ 1024px (default)  | `quality`             | `max(40, quality−25)` |
| `lg`  | ≤ 1440px (default)  | `quality`             | `max(40, quality−25)` |
| `org` | ≤ 1920px (default)  | 95 (near-lossless)    | 70 |

- **No upscaling.** A 500px source stays 500px across every variant.
- **AVIF is always on** — there is no toggle. It is the modern-format payload; JPEG/PNG is the universal fallback. WebP is decode-only (accepted as input, never produced).
- **`org` is the largest variant, capped at 1920px by default** — *not* full resolution. This keeps the heaviest AVIF encode bounded so it's fast everywhere, no special setup. Want true originals? Set `sizes.org: false` to disable the cap entirely (full source resolution), or `sizes.org: <number>` for a different cap. If `org` re-encodes *larger* than the source (already-optimized input), the pipeline keeps the original bytes instead.
...
// }
//
// Store it as a single JSON/JSONB column. The per-variant w/h are what kill
// layout shift (CLS) — the <img> reserves space before bytes arrive.
```

### 1. Client — optimize and upload all 8 blobs

```ts
import { squash, type OptimizedImage } from "browser-image-squash";

type VariantKey = "sm" | "md" | "lg" | "org";
const KEYS: VariantKey[] = ["sm", "md", "lg", "org"];

export async function optimizeAndUpload(file: File): Promise<OptimizedImage> {
  // 1. Optimize in the browser → 8 blobs.
  const result = await squash(file);

  // 2. Flatten to an upload list. AVIF is always present.
  const rasterExt = result.format === "png" ? "png" : "jpg";
  const parts = KEYS.flatMap((key) => {
    const v = result.variants[key];
    return [
      { field: `${key}.${rasterExt}`, blob: v.raster.blob, key, kind: "raster" as const, w: v.raster.width, h: v.raster.height },
      { field: `${key}.avif`,         blob: v.avif.blob,   key, kind: "avif"   as const, w: v.avif.width,   h: v.avif.height   },
    ];
  });

  // 3. Upload as one multipart POST (simplest). For many files at once, cap
  //    concurrency yourself (see the batch note below) so you don't flood the pool.
  const form = new FormData();
  for (const p of parts) form.append(p.field, p.blob, p.field);
  const stored: Record<string, string> = await fetch("/api/images", {
    method: "POST",
    body: form,
  }).then((r) => r.json()); // { "sm.jpg": "https://…", "sm.avif": "https://…", … }

  // 4. Assemble the OptimizedImage from returned URLs.
  const variant = (key: VariantKey) => {
    const r = parts.find((p) => p.key === key && p.kind === "raster")!;
    const a = parts.find((p) => p.key === key && p.kind === "avif")!;
    return { w: r.w, h: r.h, url: stored[r.field], avifUrl: stored[a.field] };
  };
  return {
    v: 1,
    format: result.format,
    sm: variant("sm"), md: variant("md"), lg: variant("lg"), org: variant("org"),
  };
}
```

> For large batches (20 photos × 8 blobs = 160 PUTs) cap concurrency so you don't open 160 sockets at once — chunk your `Promise.all`, or use a tiny limiter such as [`p-limit`](https://github.com/sindresorhus/p-limit):
> ```ts
> import pLimit from "p-limit";
> const limit = pLimit(12); // module-level, app-wide
> await Promise.all(parts.map((p) => limit(() => putBlob(p))));
> ```
...
app.post("/api/images", upload.any(), async (req, res) => {
  ...
  for (const f of files) {
    // f.fieldname is "sm.jpg" / "sm.avif" / … — keep it as the object key.
